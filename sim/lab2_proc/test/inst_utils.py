...
#-------------------------------------------------------------------------
# gen_rr_stall_template
#-------------------------------------------------------------------------

def gen_rr_stall_template(
  num_nops_inst1src0, num_nops_inst1src1, num_nops_inst2src1, 
  num_nops_dest,
  inst1_reg_src0, inst1_reg_src1,
  inst2_reg_src1,
  inst, inst1_src0, inst1_src1, inst2_src1, result
):
  return """

    # Move instruction 1 src0 value into register
    csrr {inst1_reg_src0}, mngr2proc < {inst1_src0}
    {nops_inst1_src0}

    # Move instruction 1 src1 value into register
    csrr {inst1_reg_src1}, mngr2proc < {inst1_src1}
    {nops_inst1_src1}

    # Move instruction 2 src1 value into register
    csrr {inst2_reg_src1}, mngr2proc < {inst2_src1}
    {nops_inst2_src1}

    # Instruction under test
    {inst} x3, {inst1_reg_src0}, {inst1_reg_src1}
    {nops_dest}

    # Instruction under test
    {inst} x4, x3, {inst2_reg_src1}
    {nops_dest}
  ...
  """.format(
    nops_inst1_src0 = gen_nops(num_nops_inst1src0),
    nops_inst1_src1 = gen_nops(num_nops_inst1src1),
    nops_inst2_src1 = gen_nops(num_nops_inst2src1),
    nops_dest = gen_nops(num_nops_dest),
    **locals()
  )

#-------------------------------------------------------------------------
# gen_rr_stall_value_test
#-------------------------------------------------------------------------

def gen_rr_stall_value_test( inst, inst1src0, inst1src1, inst2src1, result ):
  return gen_rr_stall_template( 0, 0, 0, 0, "x1", "x2", "x5",
                                inst, inst1src0, inst1src1, inst2src1, result )
...
#vary number of nops
def gen_jal_dest_dep_test(num_nops, inst, result):
  return gen_jal_template(num_nops, inst, result)

#test the correct jump resolution
def gen_jal_value_test(inst, result):
  return gen_jal_template(0, inst, result)
...
#vary number of nops inserted between writing the src reg and reading the reg in the inst
def gen_jalr_src_dep_test(num_nops, inst, result):
  return gen_jalr_template(num_nops, 0, "x1", inst, result)

#vary number of nops
def gen_jalr_dest_dep_test(num_nops, inst, result):
  return gen_jalr_template(0, num_nops, "x1", inst, result)

#test correct branch resolution
def gen_jalr_value_test(inst, result):
  return gen_jalr_template(0,0,"x1", inst, result)
...
#-------------------------------------------------------------------------
def gen_sw_template(
  num_nops_base, num_nops_dest, rs1_reg, rs2_reg, offset, rs1, rs2
):
  return """
    csrr {rs1_reg}, mngr2proc < {rs1}
    csrr {rs2_reg}, mngr2proc < {rs2}
    {nops_base}

    sw {rs2_reg}, {offset}({rs1_reg})
    {nops_dest}

    csrr x4, mngr2proc < {rs1}
    lw x3, {offset}(x4)
    csrw proc2mngr, x3 > {rs2}
...
#-------------------------------------------------------------------------
def gen_sw_dest_dep_test( num_nops, rs1, rs2):
  return gen_sw_template(8, num_nops, "x1", "x2", 0, rs1, rs2)

#-------------------------------------------------------------------------
# gen_sw_base_dep_test
#-------------------------------------------------------------------------
def gen_sw_base_dep_test( num_nops, rs1, rs2 ):
  return gen_sw_template( num_nops, 0, "x1", "x2", 0, rs1, rs2)

#-------------------------------------------------------------------------
# gen_sw_base_eq_dest_test
#-------------------------------------------------------------------------
def gen_sw_base_eq_dest_test(rs1, rs2 ):
  return gen_sw_template( 0, 0, "x1", "x2", 0, rs1, rs2)

#-------------------------------------------------------------------------
# gen_sw_value_test
#-------------------------------------------------------------------------
def gen_sw_value_test( offset, rs1, rs2 ):
  return gen_sw_template( 0, 0, "x1", "x2", offset, rs1, rs2 )

#-------------------------------------------------------------------------
# auipc template
#-------------------------------------------------------------------------

def gen_auipc_template( num_base_nops, num_dest_nops, rs1_reg, imm, result ):
  return """
    auipc {rs1_reg}, {imm}
    {base_nops}
    csrw proc2mngr, {rs1_reg} > {result}
    {dest_nops}
  """.format(
    base_nops = gen_nops(num_base_nops),
    dest_nops = gen_nops(num_dest_nops),
    **locals() )

#-------------------------------------------------------------------------
# lui template
#-------------------------------------------------------------------------
def gen_lui_template(num_base_nops, num_dest_nops, rs1_reg, imm, result ):
  return """
    lui {rs1_reg}, {imm}
    {base_nops}
    csrw proc2mngr, {rs1_reg} > {result}
